...
//warning message
  ...
  //JSX code start from here
  return (
    <Container fluid className={styles.container}>
<Row className={styles.container}>
<Col className={`${styles.colContainerLeft} ${styles.leftBackground}`}></Col>
         <Col className={styles.colContainerRight}>
         <ToastContainer />
      <Formik
        validationSchema={schema} initialValues={{email: "",password: "",firstName: "", lastName: "",  }} onSubmit={(values, { setSubmitting }) => { postSignUpInfo(values); setSubmitting(false); }} >
        {({
          ...
          <Form  noValidate onSubmit={handleSubmit} className={styles.formContainer} >
            ...
            <Button type="submit" variant="success" className={`${styles.btnSignUp} mb-3`}>
              Sign Up  
            </Button>
          </Form>
        )}
